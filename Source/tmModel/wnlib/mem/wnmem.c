...
local void lo_trace_group_one_line(FILE *out, wn_memgp group)
{
  ...
  /**/  percent_leftover, wn_heapsize(), group->label ? group->label : "");
  fflush(out);
...
local void lo_trace_group_free_start(wn_memgp group)
{
  ...
  new_heapsize = wn_heapsize();
  if (new_heapsize < lo_last_heapsize)
  {
    fprintf(lo_trace_group_fp,
    /**/  "Drop at START of group free, by %13lu %14lu %s\n",
    /**/  lo_last_heapsize - new_heapsize, new_heapsize,
    /**/  group->label ? group->label : "");
  }
  lo_last_heapsize = new_heapsize;
...
void wn_gp_pad(void)
{
# if (WN_SIZEOF_PTR > WN_SIZEOF_INT)
    fprintf(stderr,
